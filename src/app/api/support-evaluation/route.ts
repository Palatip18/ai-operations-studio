...
import { runSupportEvaluationSuite } from "@/lib/support-evaluation";

export async function GET(request: Request) {
  if (!isAuthenticatedRequest(request)) return NextResponse.json({ error: "Authentication required." }, { status: 401 });
  const rate = checkRateLimit(request, "support-evaluation", 5, 10 * 60 * 1000);
  ...
  return NextResponse.json(await runSupportEvaluationSuite());
}
