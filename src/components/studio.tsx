...
import { LanguageSwitcher, uiCopy, useUiLocale, type UiLocale } from "@/lib/ui-i18n";
...
const moduleIds: Module[] = ["chat", "knowledge", "workflow", "agent", "support"];

export function Studio() {
  const { locale, setLocale, copy } = useUiLocale();
  const modules = moduleIds.map((id, index) => ({ id, number: String(index + 1).padStart(2, "0"), label: copy.modules[index][0], description: copy.modules[index][1] }));
    ...
    <div className="mx-auto min-h-screen max-w-[1500px] px-5 py-5 sm:px-8 lg:px-12">
      <header className="flex items-center justify-between border-b border-white/10 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div><p className="text-sm font-semibold tracking-tight">AI Operations Studio</p><p className="text-xs text-[#90a9a0]">{copy.subtitle}</p></div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          <LanguageSwitcher locale={locale} onChange={setLocale} />
          <div className="hidden items-center gap-2 rounded-full border border-green-300/20 bg-green-300/5 px-3 py-1.5 text-xs text-green-200 sm:flex"><span className="h-1.5 w-1.5 rounded-full bg-green-300" />{liveAI ? copy.live : copy.safe}</div>
          <button type="button" onClick={signOut} className="rounded-full border border-white/10 bg-white/[.03] px-3 py-1.5 text-xs text-[#90a9a0] transition hover:border-green-300/30 hover:text-green-200">{copy.signOut}</button>
        </div>
      </header>

      <main className="grid gap-10 py-10 lg:grid-cols-[340px_1fr] lg:py-16">
        <section>
          <p className="mb-4 font-mono text-xs uppercase tracking-[.22em] text-green-300">{copy.eyebrow}</p>
          <h1 className="max-w-sm text-4xl font-semibold leading-[1.05] tracking-[-.05em] sm:text-5xl">{copy.hero}</h1>
          <p className="mt-5 max-w-sm text-sm leading-6 text-[#90a9a0]">{copy.intro}</p>
        ...
        <section className="min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-[#0c1917]/90 shadow-2xl shadow-black/30">
          <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
            <div><p className="text-sm font-medium">{modules.find((m) => m.id === active)?.label}</p><p className="text-xs text-[#718a81]">{copy.interactive}</p></div>
            <span className="rounded-md bg-white/5 px-2 py-1 font-mono text-[10px] uppercase tracking-wider text-[#718a81]">{copy.prototype}</span>
          </div>
          <div className="min-h-[570px] p-5 sm:p-7">{active === "chat" ? <ChatDemo /> : active === "knowledge" ? <KnowledgeDemo /> : active === "workflow" ? <WorkflowDemo /> : active === "agent" ? <AgentDemo /> : <SupportDemo locale={locale} />}</div>
        </section>
      </main>
      <footer className="flex flex-col gap-2 border-t border-white/10 py-5 text-xs text-[#60776f] sm:flex-row sm:items-center sm:justify-between"><p>{copy.footer}</p><p className="font-mono">Next.js · TypeScript · Tailwind CSS</p></footer>
    </div>
  );
}
...
function SupportDemo({ locale }: { locale: UiLocale }) {
  ...
  const localizedScenarios = copy.scenarios.map(([label, message], index) => ({ label: `${index + 1} · ${label}`, message }));
  const [message, setMessage] = useState("");
  const [submittedMessage, setSubmittedMessage] = useState("");
  ...
  return <div className="flex min-h-[510px] flex-col">
    <div className="mb-5 rounded-xl border border-white/10 bg-[#07100f] p-4">
      <p className="mb-2 font-mono text-[10px] uppercase tracking-wider text-green-300">{copy.supportTitle}</p>
      <p className="text-sm leading-5 text-[#b7cbc3]">{copy.supportIntro}</p>
      <p className="mt-2 text-xs text-[#718a81]">{copy.supportLimit}</p>
    </div>
    <div className="mb-6 flex flex-wrap gap-2">{localizedScenarios.map((scenario) => <button key={scenario.label} type="button" disabled={loading} onClick={() => run(scenario.message)} className="rounded-full border border-green-300/20 bg-green-300/5 px-3 py-1.5 text-left text-xs text-green-200 transition hover:border-green-300/40 disabled:opacity-50">{scenario.label}</button>)}</div>

    <div className="flex-1 space-y-4" aria-live="polite">
      {!submittedMessage && <div className="rounded-xl border border-dashed border-white/10 px-5 py-10 text-center"><p className="text-sm text-[#90a9a0]">{copy.supportEmpty}</p><p className="mt-2 text-xs text-[#60776f]">{copy.fictional}</p></div>}
      {submittedMessage && <div className="ml-auto max-w-[85%] rounded-2xl rounded-br-sm bg-green-300 px-4 py-3 text-sm text-[#07100f]">{submittedMessage}</div>}
      {loading && <div className="max-w-[90%] rounded-2xl rounded-bl-sm border border-white/10 bg-white/[.04] px-4 py-3 text-sm text-[#90a9a0]">{copy.processing}</div>}
      {result && <div className="max-w-full space-y-3">
        ...
        <div className="rounded-xl border border-white/10 bg-[#07100f] p-4">
          ...
          <div className="mb-4 grid grid-cols-2 gap-2 text-xs">
            <p className="text-[#b7cbc3]">{copy.intent}: <span className="text-green-300">{result.trace.intent}</span></p>
            <p className="text-[#b7cbc3]">{copy.risk}: <span className={result.trace.risk === "HIGH" ? "text-amber-200" : "text-green-300"}>{result.trace.risk}</span></p>
          </div>

          {result.trace.escalationReason && <p className="mb-4 rounded-lg border border-amber-300/30 bg-amber-300/10 p-3 text-xs text-amber-100">{copy.reason}: {result.trace.escalationReason}</p>}
    ...
    <form onSubmit={submit} className="mt-6 flex gap-2 rounded-xl border border-white/10 bg-[#07100f] p-2"><input aria-label="Support message" value={message} onChange={(e) => setMessage(e.target.value)} placeholder={copy.placeholder} className="min-w-0 flex-1 bg-transparent px-2 text-sm outline-none placeholder:text-[#456057]" /><button disabled={loading || !message.trim()} className="rounded-lg bg-green-300 px-4 py-2 text-sm font-medium text-[#07100f] disabled:opacity-50">{loading ? copy.running : copy.run}</button></form>
  </div>;
}
